...
interface WeatherData {
  current_weather: { [key: string | number]: number };
  daily: { [key: string | number]: number[] };
  hourly: { [key: string | number]: number[] };
}

function parseCurrentWeather({ current_weather, daily }: WeatherData) {
  const {
...
function parseDailyWeather({ daily }: WeatherData) {
...
function parseHourlyWeather({ hourly, current_weather }: WeatherData) {
  return hourly.time
    .map((time, index) => {
      return {
        timestamp: time * 1000,
